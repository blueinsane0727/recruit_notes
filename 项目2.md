...
各家差异：
- μC/OS-II 用 OSRdyTbl 位图 + 优先级表、O(1) 选任务；
- FreeRTOS 用 就绪链表数组 + uxTopReadyPriority 位图 + CLZ；
- RT-Thread 用对象容器（链表挂所有内核对象）+ 线程优先级 + 时间片；
- Zephyr 用 rbtree/多链表 + 抢占阈值。
...
### 7.对外提供哪些 API？任务、队列、信号量、互斥量、内存管理分别有哪些？

- 任务：xTaskCreate/xTaskCreateStatic/vTaskDelete/vTaskDelay/vTaskDelayUntil/xTaskDelayUntil/vTaskSuspend/vTaskResume/xTaskResumeFromISR/uxTaskPriorityGet/vTaskPrioritySet/uxTaskGetStackHighWaterMark/vTaskList/vTaskGetRunTimeStats/taskYIELD。
- 队列：xQueueCreate/xQueueCreateStatic/xQueueSend(=ToBack)/xQueueSendToFront/xQueueOverwrite/xQueueReceive/xQueuePeek/xQueueSendFromISR/xQueueReceiveFromISR/uxQueueMessagesWaiting/uxQueueSpacesAvailable/xQueueReset。
- 信号量：xSemaphoreCreateBinary/xSemaphoreCreateCounting/xSemaphoreTake/xSemaphoreGive/xSemaphoreGiveFromISR/xSemaphoreTakeFromISR。
- 互斥量：xSemaphoreCreateMutex/xSemaphoreCreateRecursiveMutex/xSemaphoreTakeRecursive/xSemaphoreGiveRecursive（互斥量**没有** FromISR 版本）。
- 内存：pvPortMalloc/vPortFree/xPortGetFreeHeapSize/xPortGetMinimumEverFreeHeapSize

### 8.可移植性怎么设计？移植层包含哪些文件？

FreeRTOS 的移植接口只有固定一小撮符号，核心 C 文件完全不碰寄存器：
- `portmacro.h`（`StackType_t/BaseType_t/TickType_t`、`portSTACK_GROWTH`、`portYIELD`、`portENTER_CRITICAL/EXIT`、`portSET_INTERRUPT_MASK_FROM_ISR`）；
- `port.c`（`pxPortInitialiseStack`、`xPortStartScheduler`、`vPortStartFirstTask`、`vTaskSwitchContext` 所在的 `PendSV_Handler`、`xPortSysTickHandler`、`vPortSetupTimerInterrupt`、`pvPortMalloc` 相关钩子），某些端口另有 `portasm.s`；
- `FreeRTOSConfig.h`（裁剪与时钟参数）。
即：**移植 = 提供“启动第一个任务 + 触发切换 + 临界区屏蔽 + 栈初始化 + tick 源”这五件事**，其余代码零改动。
...
M3 的“移植友好点”：有 CLZ（可 O(1) 选最高优先级就绪任务）、有硬件 SDIV/UDIV 与单周期乘法、标准 4 位优先级、ARMv7-M 完整的 Thumb-2 指令集，还没有 FPU 带来的浮点上下文问题。
...
FreeRTOS 里 `configMAX_PRIORITIES = 32`，优先级 **0 = 最低（`tskIDLE_PRIORITY`），`configMAX_PRIORITIES-1` = 最高**，数值越大越优先（与 μC/OS 相反，要说清）。RAM 开销可以直接算：
- `List_t` = `uxNumberOfItems(4) + pxIndex(4) + xListEnd(12) = 20` 字节；
- `pxReadyTasksLists[32]` = **640 字节**，加 `uxTopReadyPriority`（`UBaseType_t`，4 字节）= **约 644 字节**；
- 再乘上 `configUSE_LIST_DATA_INTEGRITY_CHECK_BYTES` 的调试冗余（每个 List 再加 8 字节）。
所以结论是：**优先级档位是“用 RAM 换查找速度”**，32 级比 8 级白白多花 ~480 字节，默认配置里 `configMAX_PRIORITIES` 常取 5~7 就够了。

### 12.有没有空闲任务？空闲任务做什么？

有，`vTaskStartScheduler()` 强制创建，优先级固定 0（最低）。`prvIdleTask()` 循环做四件事：
- `prvCheckTasksWaitingTermination()`：**回收自杀任务**的 TCB 与栈（`vPortFree(pxStack)` + `vPortFree(pxTCB)`）；
...
面试提醒：空闲任务是“必须存在且不可删除”的（删了就没人为自杀任务收尸，FreeRTOS 会在 `vTaskDelete` 里 `configASSERT`），并且它的栈由 `configMINIMAL_STACK_SIZE` 决定，太小会在空闲钩子里爆栈。

### 13.有没有断言、错误码、HardFault 处理？

三层机制，FreeRTOS 的做法可以照搬：
- **断言**：`configASSERT( x )` 由用户定义（典型实现：`if(!(x)){ taskDISABLE_INTERRUPTS(); for(;;); }` 或直接 `__BKPT()`），内核到处在用——ISR 优先级校验、队列/互斥量用法校验、调度器状态校验全靠它。**开发期必须打开，量产后才关**。
- **错误码/钩子**：API 返回 `pdPASS/pdFAIL`、`errQUEUE_FULL`、`errQUEUE_EMPTY`、`errCOULD_NOT_ALLOCATE_REQUIRED_MEMORY`；钩子有 `vApplicationMallocFailedHook()`、`vApplicationStackOverflowHook(xTask, pcTaskName)`、`vApplicationIdleHook()`、`vApplicationTickHook()`、`configUSE_DAEMON_TASK_STARTUP_HOOK`。
- **HardFault**：内核不提供，自己写 `HardFault_Handler`——由 `EXC_RETURN` 判断用的是 MSP 还是 PSP，取出压栈帧里的 PC/LR/xPSR，读 `CFSR/HFSR/BFAR/MMFAR` 判定原因，再打印 `pxCurrentTCB->pcTaskName` 定位是哪个任务出的错。

## 二、任务管理与调度器

### 15.任务控制块 TCB 包含哪些字段？每个字段作用是什么？

FreeRTOS `TCB_t` 的关键字段（32 位机上默认配置约 90~100 字节，随配置变化）：
- `pxTopOfStack`：保存的栈顶值，切换时被 PendSV 读取；
- `xStateListItem`（**20 字节**）：挂就绪/延时/挂起/等待终止链表；其 `xItemValue` 在不同状态下含义不同——**就绪态存优先级、延时态存唤醒 tick**；
- `xEventListItem`（**20 字节**）：挂事件（队列/信号量/事件组）阻塞链表，`xItemValue = configMAX_PRIORITIES - uxPriority`（借列表的升序实现“高优先级在前”）；
- `uxPriority`（当前生效优先级）与 `uxBasePriority`（原始优先级，**只有互斥量继承会动它**）；
- `uxMutexesHeld`：持有的互斥量个数（释放时用来决定回落到哪个优先级）；
- `pxStack`：栈起始地址（用于溢出检查与释放）；
- `pcTaskName[configMAX_TASK_NAME_LEN]`：默认 16 字节；
- `ulNotifiedValue[]` 与 `ucNotifyState[]`：任务通知的值与状态；
- `uxTaskNumber/uxTCBNumber`：调试与追踪编号（`vTaskList` 用）；
- `ulRunTimeCounter`：运行时间统计累加器；
- `uxThreadLocalStoragePointers[]`、`pxTaskTag`、`ucStaticallyAllocated`、`ucDelayAborted`；
- 端口私有部分：`portCRITICAL_NESTING_IN_TCB == 1` 时每个 TCB 内自带 `uxCriticalNesting`，以及 `configUSE_TASK_FPU_SUPPORT` 相关的 FPU 标志位。

### 16.任务有哪些状态？状态迁移图怎么画？

FreeRTOS 的“真实”状态只有四个（`eTaskGetState()` 的取值范围）：**Running / Ready / Blocked / Suspended**，外加一个 `eDeleted`（已被删除、等空闲任务收尸）。**FreeRTOS 没有“New（新建）”状态**——`xTaskCreate()` 完成后任务直接进就绪表，这点和 μC/OS 的 DORMANT→READY 五态图不同，面试时要说清差别。
迁移边：Ready↔Running（被抢占/被选中）；Running→Blocked（`vTaskDelay` 或等待事件），Blocked→Ready（tick 到期或在延时表被摘除时事件满足）；Running/Ready→Suspended（`vTaskSuspend`，**挂起态不参与调度、也不被 tick 计超时**），Suspended→Ready（`xTaskResumeFromISR`）；任意态→Deleted。
关键细节：**一个任务同一时刻可能同时挂在两张链表上**——被 `vTaskSuspend` 的任务若同时阻塞在队列上，它既在 `xSuspendedTaskList` 又在事件链表里，恢复时要 `xTaskResumeFromISR` 返回真才真正就绪（这正是 `xPendingReadyList` 存在的原因）。
...
关键点：环形链表 + `pxIndex` 这两个设计一起，才让“找下一个同优先级任务”是 O(1)，不需要额外计数器或指针数组。

### 18.如何快速找到最高优先级任务？用 CLZ 指令吗？时间复杂度多少？

两种实现，靠 `configUSE_PORT_OPTIMISED_TASK_SELECTION` 切换：
- **=1（Cortex-M3 默认推荐）**：`portGET_HIGHEST_PRIORITY(uxTopPriority, uxTopReadyPriority)` 展开为 `__clz(uxTopReadyPriority)`，用 `31 - clz` 取最高置位 → **O(1)**；这也要求 `configMAX_PRIORITIES <= 32`（一个 `UBaseType_t` 装得下所有优先级位）。
- **=0（M0 等无 CLZ 平台）**：从 `uxTopReadyPriority` 起**向下循环**找第一条非空就绪链表——**O(configMAX_PRIORITIES)**，最坏 32 次循环。
注意 `uxTopReadyPriority` 只是一个“上界提示”：任务从链表摘除时不一定立刻把对应位清零（为了省掉每次查找），所以通用版本必须“先信位图、再验链表非空”，这就是那个 `while(listLIST_IS_EMPTY(...)) uxTopPriority--;` 循环的由来。

### 19.同优先级任务的时间片轮转怎么实现？

**不需要时间片计数器**，靠“环形链表的 pxIndex 每次切换都前进一格”实现。`vTaskSwitchContext()`：
...
- 若当前任务仍是最高的，但同优先级链表长度 > 1（`configUSE_TIME_SLICING == 1`），`xTaskIncrementTick()` 会返回 `pdTRUE` 触发一次切换 → 走的就是上面这条路径 → 轮到同优先级的下一个任务。
所以“时间片”= **每次 tick 都做一次同优先级轮转**，切走时 `pxIndex` 恰好停在切走的任务上，下次就取到它后面那个。

### 20.时间片大小怎么定？是否可配置？

FreeRTOS 里时间片**恒等于 1 个 tick**（`1/configTICK_RATE_HZ`），没有“每个任务可设时间片长度”的 API——只能通过两个全局宏改变语义：
- `configUSE_TIME_SLICING = 0` → 完全关闭轮转（同优先级任务一旦运行就跑到主动让出为止）；
- `configTICK_RATE_HZ`（常取 100/1000）→ 改变时间片粒度，代价是 tick 中断开销与延时分辨率。
对比：μC/OS-III、RT-Thread 支持**每线程时间片**（`rt_thread_control(RT_THREAD_CTRL_CHANGE_TIMESLICE)`）；若自研内核要支持，就得在 TCB 里加 `uxTimeSlice/uxTimeSliceRemain`，并在 tick 里递减、为 0 才轮转——这正是你回答“我的内核相比 FreeRTOS 的改进点”的好素材。

### 21.SysTick 中断里具体做了什么？如何更新延时和唤醒？

`xPortSysTickHandler()` 本体极薄，且**整个 tick 逻辑都在临界区里**：
`portDISABLE_INTERRUPTS()` → `if(xTaskIncrementTick() != pdFALSE) ICSR = PENDSVSET` → `portENABLE_INTERRUPTS()`。
`xTaskIncrementTick()` 做的事：
- 若 `uxSchedulerSuspended == 0`：`xTickCount++`（溢出到 0 时 `taskSWITCH_DELAYED_LISTS()` 交换两个延时链表）；若 `xTickCount >= xNextTaskUnblockTime`，则**循环**从延时链表头摘除所有 `xItemValue <= xTickCount` 的任务——顺手 `uxListRemove(&pxTCB->xEventListItem)`（若非 NULL，说明它同时在等事件），再 `prvAddTaskToReadyList()` 把它挂回就绪表，并更新 `xNextTaskUnblockTime`；然后判断是否需要切换（有更高优先级就绪 → 需要；或同优先级就绪 >1 且开启时间片 → 需要）；
- 若调度器被挂起（`vTaskSuspendAll`）：`xPendedTicks++`，**先记账不切**，等 `xTaskResumeAll()` 时再把积压的 tick 补算并统一处理。
最后调用 `vApplicationTickHook()`（`configUSE_TICK_HOOK`）。

### 22.延时链表怎么组织？按唤醒时间排序插入吗？插入复杂度多少？

FreeRTOS 用 **两条延时链表 + 两个指针**：`xDelayedTaskList1/xDelayedTaskList2`，由 `pxDelayedTaskList`（当前）与 `pxOverflowDelayedTaskList`（用于 tick 溢出后）轮流指向，换取“tick 回绕时有序性自动成立”，避免为回绕做特殊排序。
组织方式：`xStateListItem.xItemValue = 绝对唤醒 tick`，用 `vListInsert()` **按值升序插入**（不是尾插），所以表头永远是最近要醒的任务，`xNextTaskUnblockTime` 就是它的值，tick 中断里只需要和这个缓存值比一次就能快速跳过。
- 插入复杂度：**O(n)**（n = 已延时任务数），平均很短（任务是按唤醒顺序插入的，多数情况落在表尾附近）；
- 一个重要的例外：`xTicksToWait == portMAX_DELAY` 且允许无限阻塞的任务**根本不进延时链表**（只在事件链表里挂着），这样“永久阻塞”的任务不会拖慢延时链表的插入与遍历。

### 23.阻塞延时和定时唤醒有什么区别？

两者最终都落到 `prvAddCurrentTaskToDelayedList()`，区别在“靠谁唤醒”：
- **纯延时（阻塞延时）**：只挂延时链表，唤醒源唯一 —— tick 到期（`vTaskDelay` / `vTaskDelayUntil`）。`vTaskDelayUntil` 用绝对时刻算延时量，可消除累计抖动，适合周期任务。
- **带超时的等待（定时唤醒）**：同时挂在**事件链表 + 延时链表**两处，**谁先到谁生效**；事件先到就由 `xTaskRemoveFromEventList` 把它从延时链表里摘掉（这就是 `pxTCB->xStateListItem.xItemValue != 0` 才 `uxListRemove` 的原因）；tick 先到就由 tick 循环把它从事件链表里摘掉，并返回超时结果（`errQUEUE_EMPTY` 或 `pdFALSE`）。
因此“超时”和“唤醒”在实现上是**同一套双挂结构**的两种先到者胜，不是两套机制。

### 24.任务创建时栈怎么分配？栈增长方向是什么？

动态创建时是**两次独立分配**：`pxNewTCB = pvPortMalloc(sizeof(TCB_t))`，`pxStack = pvPortMalloc(configSTACK_DEPTH * sizeof(StackType_t))`（`StackType_t = uint32_t`），所以一次 `xTaskCreate(T, "n", 128, ...)` 只申请 512 字节栈。
栈是 **满递减（Full Descending）**：`pxTopOfStack = pxStack + configSTACK_DEPTH - 1`，从高地址往低地址长，因为 Cortex-M 的 `PUSH/STMDB` 就是先减后存。初始化时 `pxPortInitialiseStack()` 在栈顶预置一个“假的异常压栈帧”：`xPSR = 0x01000000`（**Thumb 位**，M3 必须置 1，忘了会跑进 HardFault）、`PC = 任务入口`、`LR = prvTaskExitError`（任务函数一旦返回就落进这个陷阱）、`R12/R3/R2/R1`、`R0 = pvParameters`，再接 `R11..R4`。
静态创建（`xTaskCreateStatic`）时两块内存都是用户给的数组，并且要求**栈数组的“顶部”必须是高地址端**——这点是静态分配端口最容易写错的地方。

### 25.任务删除流程是什么？TCB、栈、队列阻塞节点怎么回收？

`vTaskDelete(pxTaskToDelete)` 分两种情况：
- **删别人（或删已挂起/阻塞的任务）**：`uxListRemove(&xStateListItem)` 摘掉（就绪/延时/挂起表任一条）→ **若 `xEventListItem` 的容器非空（即该任务正阻塞在某个队列/信号量上）也要 `uxListRemove` 摘掉** → `prvDeleteTCB()` 直接释放 `pxStack` 与 `pxTCB`。注意：**它留在队列里的“阻塞节点”必须在这里摘干净**，否则队列解锁时会解引用一块已释放内存。
- **删自己（`vTaskDelete(NULL)`）**：**不能立刻释放自己的栈**（还在用），所以 `listINSERT_END(&xTasksWaitingTermination, &pxTCB->xStateListItem)` + `uxDeletedTasksWaitingCleanUp++`，然后 `portYIELD_WITHIN_API()` 切走；由**空闲任务**的 `prvCheckTasksWaitingTermination()` 事后 `prvDeleteTCB()` 收尸。
- 记忆点：**“自杀靠空闲任务收尸”是 FreeRTOS 的核心设计**，所以空闲任务不可删、且 `INCLUDE_vTaskDelete` 时可删任务的数量受空闲任务调度频率影响（空闲任务一直没机会跑，内存就一直不还）。

### 26.如果任务删除自身，调度器怎么返回？

**不返回**——这是关键。`vTaskDelete(NULL)` 把自己挂进 `xTasksWaitingTermination` 后调用 `portYIELD_WITHIN_API()`（置 PendSV），随后的调度过程从就绪表里选出的必然是别的任务，`pxCurrentTCB` 被换掉；等到某个时刻 `vTaskSwitchContext` 若能选回这个已删任务，那它早已不在任何就绪链表里，**永远不会被选中**，因此它的上下文（PendSV 里保存的 `pxTopOfStack`）虽还在 TCB 里，却再也不会被 `ldmia` 恢复。
两个必须点到的细节：①`vTaskDelete` 的 `portYIELD_WITHIN_API()` 是在**临界区内**调用的，保证“入表 + 触发切换”原子完成，不会被 tick 中断打断；②任务一旦调用它，**后面不能再访问自己的栈/局部变量**（栈随时可能被空闲任务释放），所以它必须是任务体的最后一条语句。这种“栈被异步回收”的危险性正是答“我的内核会怎么改进”的切入点（例如改成延迟到下一次切换后再释放，或要求任务返回而非自删）。

### 27.如果删除持有互斥量的任务，会发生什么？

**死锁 + 悬垂指针，双料灾难**，FreeRTOS 官方文档明确警告：
- 互斥量的 `xMutexHolder` 指向已释放的 TCB 内存，`xMutexesHeld`、`uxBasePriority` 随 TCB 一起消失 → 互斥量永远处于“被持有”状态，`uxMessagesWaiting == 0`，**任何后继 `xSemaphoreTake` 都将永久阻塞**；
- 更糟的是下一次有任务阻塞在这个互斥量上时，`xTaskPriorityInherit(pxQueue->u.xSemaphore.xMutexHolder)` 会**解引用已经 `vPortFree` 掉的 TCB**——若这块内存被重新分配（比如新任务），就会去改“别人的优先级”，属于典型的 **use-after-free 静默数据损坏**，这类 bug 极难查。
- 正确做法：①绝不在持有互斥量时删除任务；②改用**看门狗/软定时器 + 超时值**（`xSemaphoreTake(m, pdMS_TO_TICKS(100))`）让阻塞方可超时退出；③真需要“删除即释放”，得在内核层面实现 `vTaskDelete` 时扫描 `uxMutexesHeld` 并强制释放（μC/OS 的思路），FreeRTOS 不做这件事。

### 28.任务优先级能否动态修改？修改后就绪列表怎么调整？

可以，`vTaskPrioritySet(xTask, uxNewPriority)`。FreeRTOS 的处理逻辑（面试要能说全）：
- 若目标是当前任务且在跑：`pxCurrentTCB->uxPriority = uxNewPriority`；若它**持有互斥量**（`uxMutexesHeld > 0`），则**同步更新 `uxBasePriority` 与所有已持有互斥量事件项的 `xItemValue`**（`configMAX_PRIORITIES - newPrio`），否则继承关系会错乱；
- 若目标在就绪态：先从原优先级就绪链表 `uxListRemove`，`prvAddTaskToReadyList()` 插到新链表，若新优先级高于当前任务优先级则 `portYIELD_WITHIN_API()` 立即切换；
- 若目标在阻塞/延时态：只改优先级与事件项值（**不摘链表**），因为它挂在事件表里的顺序由 `xItemValue` 决定，改了值顺序自动生效——这也是为什么 `xEventListItem.xItemValue` 用“优先级取反”编码；
- 限制：**降低优先级不能低于“继承来的当前优先级”**（被互斥量提升中），FreeRTOS 会把 `uxBasePriority` 一起压下去而不是拒绝，需要按你的实现说明。
加分答案：这属于“固定优先级 + 允许显式调整”，FreeRTOS **没有** EDF/RMS 这类算法调度器，实现 EDF 需要自己按 deadline 重算优先级并调用 `vTaskPrioritySet`。

### 29.空闲任务优先级是多少？什么时候运行？

优先级恒为 `tskIDLE_PRIORITY == 0`，即**最低**，且**每个调度器只有一个**。它只在“没有任何其他就绪任务”时运行（其他任务全阻塞/挂起/被删）；同优先级若有应用任务（也是 0 级），靠 `configIDLE_SHOULD_YIELD` 让出轮转。
它还有一个容易被忽略的运行时机：**即使高优先级任务在跑，空闲任务也可能被“临时唤醒”**——`vTaskResume`/`xTaskResumeAll` 把任务放进 `xPendingReadyList` 时，空闲任务本身也在就绪表里；另外 `vTaskStartScheduler` 会先让空闲任务跑起来作为兜底。

### 30.如果所有任务都阻塞，调度器怎么走？

那么最高非空就绪链表只剩 `pxReadyTasksLists[0]`，`uxTopReadyPriority` 落到 0，选中空闲任务；空闲任务在 `for(;;)` 里检查自杀任务垃圾回收、跑 `vApplicationIdleHook()`、`configUSE_TICKLESS_IDLE` 时进入休眠（`WFI` / 停 SysTick 等低功耗定时器唤醒）。tick 中断照常在跑（除非 tickless），每次 tick 检查延时链表看有没有任务该醒。
若面试官追问“空闲任务也被删了怎么办”：FreeRTOS 会在 `vTaskDelete` 里拦（空闲任务删除会导致无人回收），若真的没有任何就绪任务则调度器会选到空指针——所以内核必须保证**至少有一个永远可运行的任务**（这也是设计规范：不允许删除空闲任务）。

### 31.如何防止高优先级任务一直运行导致低优先级任务饿死？

FreeRTOS 的立场是**不做自动防饿死（starvation）保护**——固定优先级抢占式调度器在语义上就必须让高优先级持续就绪的任务一直运行实。可用的手段是“靠设计而不是靠内核”：
- **合理分层**：把长时间计算放到低优先级，把只做短触发的高优先级任务保持在 `vTaskDelay` / 等信号量的阻塞态（阻塞的高优先级任务不占 CPU）；
- **同优先级时间片**（`configUSE_TIME_SLICING`）可以解决“同优先级之间”的公平性，解决不了跨优先级；
- **主动礼让**：`taskYIELD()` 只在同优先级间有效，跨优先级需要用 `vTaskDelay(1)` 让出；
- **互斥量优先级继承**能顺带缓解“低优先级持有资源被中优先级长期抢占”的优先级反转型饿死；
- 需要严格带宽保证时应改用**非抢占式（`configUSE_PREEMPTION=0`）**——每个任务必须显式让出，天然无饿死，代价是响应时间变差（这正是 FreeRTOS 同时提供两种模式的原因）。

### 32.调度器上锁/解锁怎么实现？

`vTaskSuspendAll()` / `xTaskResumeAll()`，靠一个**计数器 `uxSchedulerSuspended`**（允许嵌套）：
- `vTaskSuspendAll()`：`++uxSchedulerSuspended`（**只关调度，不关中断**——这是它与临界区的本质区别，所以里面可以放长操作，但**绝不能调用会引起阻塞的 API**，`configASSERT` 会拦住）；
- `xTaskResumeAll()`：`--uxSchedulerSuspended`，减到 0 时把 `xPendingReadyList` 里积压的任务搬回各自就绪表、把 `xPendedTicks` 积压的 tick 补算处理（可能引发切换），最后如需要则 `portYIELD_WITHIN_API()`；返回值告诉调用者“是否已经发生了切换”。
对比：`taskENTER_CRITICAL()/taskEXIT_CRITICAL()` 是**用 BASEPRI 屏蔽中断**（也会屏蔽 PendSV，因此不会切换），适合极短操作。**内存分配器内部用的就是挂起调度器而不是临界区**（`heap_4.c` 里 `vTaskSuspendAll()/xTaskResumeAll()`），因此 `pvPortMalloc` 不能在 ISR 里调用。

### 33.中断中如何触发调度？为什么用 PendSV？

ISR 里调用任一 `...FromISR()` API 时传入 `pxHigherPriorityTaskWoken`，API 内部若唤醒了更高优先级任务就把它置 `pdTRUE`，ISR 尾部执行 `portYIELD_FROM_ISR(xHigherPriorityTaskWoken)`（本质是 `__dsb(); __isb(); ICSR = PENDSVSET;`），把切换**挂起**而不是就地切换。
用 PendSV 的三个理由（要答全）：
- **它是可“挂起（pending）”的异常**，可以在任意嵌套 ISR 里安全地登记一次切换请求，不需要区分当前中断优先级；
- **它的优先级最低**，因此切换只会发生在“所有 ISR 都退出之后”（尾链机制），避免了在 ISR 中间切换上下文导致中断嵌套结构被破坏；
- 所有上下文切换集中在**唯一一处**汇编里，硬件压栈（进入异常）与出栈（退出异常）自动完成一半工作，代码量极小，也便于分析最坏执行时间。

### 34.调度最坏执行时间大概多少？关中断时间多长？

分层回答比报单个数字更专业：
- **选任务**：`__clz` 版本 O(1)；通用版本最坏 O(configMAX_PRIORITIES) 次链表判空（32 级 → 最坏 32 次判空）；
- **上下文保存/恢复**：见第 41 题，纯汇编约 20 条指令 + `vTaskSwitchContext()` 的 C 调用（含 `configASSERT`、trace 钩子时会更长）；
- **最长的关中断区间**其实在**临界区**而不是切换里：最坏的是 `prvInsertBlockIntoFreeList()`（空闲链表按地址有序插入，O(空闲块数)）、`vListInsert()` 的延时表有序插入（O(延时任务数)）、`prvUnlockQueue()` 的阻塞链表遍历——**都和“任务/空闲块数量”线性相关，所以内核最坏执行时间是数据规模相关的，不是常数**，这一点要主动说。
- 测量方法：`DWT->CYCCNT` 在操作前后取值（Cortex-M3 的 DWT 有 32 位周期计数器，`DEMCR |= TRCENA; DWT->CTRL |= CYCCNTENA`），或用 GPIO 翻转 + 示波器；注意要先算好“计数溢出/时钟频率换算”。

### 35.栈溢出怎么检测？魔数、高水位还是硬件 MPU？

FreeRTOS 提供两种软件方法（`configCHECK_FOR_STACK_OVERFLOW`）：
- **=1（方法一）**：每次任务切换时检查 `pxTopOfStack` 是否仍落在 `[pxStack, pxStack + depth)` 区间内（即 SP 有没有越界），**开销极小但只能发现“正好在切换瞬间已经出界”的情况**，可能漏报（出界后又弹回来了）；
- **=2（方法二）**：创建任务时把整块栈填成魔数 `0xA5`（`tskSTACK_FILL_BYTE`），切换时检查**栈尾最后 20 字节**是否还被 0xA5 覆盖，被改写 → 认为溢出。**开销是启动时一次 memset**，能捕捉到更多情况，也顺带提供“用了多少”的信息；
- 触发后调用 `vApplicationStackOverflowHook(xTask, pcTaskName)`（**此时任务上下文已不可靠，只能复位或点亮指示灯**，别在里面 printf 大字符串）。
第三种是 **MPU 硬件方案**：FreeRTOS-MPU 端口给每个任务配一个在栈下方留一段“禁止访问”保护区的 MPU region，越界立即 MemManage Fault——这是唯一能精确抓到越界**写**的时刻并给出出错 PC 的做法，代价是 MPU region 数量有限（Cortex-M3 只有 8 个）。

### 36.任务栈大小怎么确定？最大栈使用量测过吗？

方法是**“先给足，再测水线，再收敛”**：
- 用 `uxTaskGetStackHighWaterMark(xTask)`（`INCLUDE_uxTaskGetStackHighWaterMark`）返回的是**历史最小剩余量（单位：字，不是字节）**——前提是栈在创建时被 `0xA5` 填充（`configCHECK_FOR_STACK_OVERFLOW == 2` 或开启 `uxTaskGetStackHighWaterMark` 时会自动 memset）；
- 拿“给的大小 − 剩余量”得到**峰值用量**，再留 30%~50% 余量（要覆盖浮点库、`snprintf`、C 库里的大栈帧、中断嵌套的临时栈——注意 **ISR 用的是 MSP，不是任务栈**，这一点很多人答错）；
- 边界条件要多测：最坏分支路径、错误路径（printf/`sprintf` 最吃栈）、递归/深层调用、所有中断同时嵌套。
参考值：空闲任务 `configMINIMAL_STACK_SIZE`（Cortex-M 常用 128 字 = 512 字节）；只做队列收发的小任务 128~256 字；用 `sprintf`/浮点的任务 512 字以上。

## 三、上下文切换与 Cortex-M3 移植

### 37.Cortex-M3 的异常模型是什么？MSP 和 PSP 怎么用？

Cortex-M3 有 **Thread 模式**（跑任务）与 **Handler 模式**（跑异常/中断）两种模式、**特权/非特权** 两级权限、**MSP/PSP 两个栈指针**：
- 复位后处于 Thread + 特权 + 使用 MSP（`CONTROL.SPSEL = 0`）；
- FreeRTOS 的约定是**“中断和内核代码用 MSP，任务用 PSP”**，切换任务时只改 PSP，MSP 始终指向主栈（在启动文件里定义的那块），所以**中断嵌套不需要切换栈指针**，ISR 的栈开销集中在 MSP 上，可以被独立评估；
- 由 `CONTROL` 寄存器第 1 位（SPSEL）选择，第 0 位（nPRIV）选择特权级。FreeRTOS 的 `vPortSVCHandler` 里写 `msr control, #2`：特权 + PSP；
- 异常返回时硬件从 `EXC_RETURN`（LR 的值）决定回哪个栈：`0xFFFFFFFD` = 回 Thread 模式用 PSP，`0xFFFFFFF9` = 回 Thread 模式用 MSP，`0xFFFFFFF1` = 回 Handler 模式。

### 38.为什么 PendSV 优先级要设最低？

因为**切换必须在“没有未处理的异常”这个干净时刻进行**：
- 若 PendSV 的优先级比某个 ISR 高，PendSV 可以在 ISR 执行到一半时切入，此时**该 ISR 的硬件压栈帧还压在（当时的）PSP 上**，`pxTopOfStack` 保存的现场会包含“被打断的 ISR 的中间状态”，一旦切走再切回，栈的增长/回收就与中断嵌套层次耦合，极易出错；
- 设为最低后，硬件保证“只有所有可抢占的异常都退出后”PendSV 才会执行（优先级相同的异常按异常号，PendSV=14 早于 SysTick=15），于是**每个任务都只在自己的、完整的上下文里被切换**；
- 由于它最低，**它自己永远不会抢占任何 ISR**，也就天然避免了“上下文切换过程中的嵌套切换”。

### 39.SVC 用来做什么？第一次任务启动怎么切到 PSP？

FreeRTOS 里 SVC 的作用就一个：**“上电式”启动第一个任务**（MPU 端口的系统调用例外）。流程在 `vPortSVCHandler` 汇编里：
`ldr r3, =pxCurrentTCB` → `ldr r1, [r3]`（取 TCB）→ `ldr r0, [r1]`（取 `pxTopOfStack`）→ `ldmia r0!, {r4-r11}`（恢复软件保存的 R4-R11）→ `msr psp, r0`（**把 PSP 指向栈帧剩余部分**）→ `mov r0, #0` → `msr basepri, r0`（清 BASEPRI，开中断）→ `orr r14, #0xd`（构造 `EXC_RETURN = 0xFFFFFFFD`）→ `bx r14`。最后一步不是普通跳转而是**异常返回**：硬件据此把 PSP 上预置的 8 个字弹出到 R0-R3/R12/LR/PC/xPSR，PC 就跳到任务入口。
因此“切到 PSP”由 `vPortSVCHandler` 一次性完成：此后 Thread 模式一直用 PSP，直到下次异常进入 Handler 模式自动用 MSP。

### 40.上下文切换保存哪些寄存器？哪些硬件自动保存？哪些手动保存？

**硬件自动压栈 8 个字（32 字节）**：`xPSR、PC、LR、R12、R3、R2、R1、R0`（进异常时按“当前使用的栈”压；若在 Thread 模式用 PSP，入栈后 `PSP -= 32`）。细节：**R0-R3 里的内容被保留在栈上而不是寄存器里**，如果中断里不调用 C 函数，可以只恢复部分（FreeRTOS 全恢复）。
**软件手动保存 8 个字**：`R4-R11`（ARM ABI 规定 R4-R11 是被调用者保存寄存器，中断整体相当于一次函数调用，所以必须由内核把关）——`stmdb r0!, {r4-r11}`。
**M4F/M7 额外**：硬件惰性压栈 `s0-s15 + FPSCR + 保留字`（`S0-S15` 才 64 字节，仅当 FPU 处于活动态才压、且可以“惰性”：直到新任务真的用 FPU 才补压），软件在 PendSV 里还要按 `EXC_RETURN` 的 bit4 决定是否 `vstmdb/vldmia {s16-s31}`——**这就是“M3 的 port.c 不能直接拿到 M4 上用”的根因**。

### 41.汇编上下文切换代码流程是什么？入栈、出栈顺序？

FreeRTOS `PendSV_Handler`（ARM_CM3）逐行流程：
1. `mrs r0, psp` + `isb` —— 取当前任务栈帧位置（**进入异常时硬件已经把 R0-R3/R12/LR/PC/xPSR 压在 PSP 上**，所以 r0 正好指向手动要压的地方）；
2. `ldr r2, =pxCurrentTCB` / `ldr r3, [r2]`；
3. `stmdb r0!, {r4-r11}` —— 手动压 R4-R11（M4F 端口这里按 `tst r14, #0x10` 条件 `vstmdb {s16-s31}`）；
4. `str r0, [r3]` —— **保存新栈顶到 `pxCurrentTCB->pxTopOfStack`**；
5. `stmdb sp!, {r3, r14}` —— 把 r3（TCB 指针）和 r14（`EXC_RETURN`）压到 MSP 上，因为下面的 C 函数会破坏它们（这就是“用 MSP 跑内核代码”的价值）；
6. `mov r0, #configMAX_SYSCALL_INTERRUPT_PRIORITY` / `msr basepri, r0` / `dsb` / `isb` —— **关中断**，保护调度器数据结构；
7. `bl vTaskSwitchContext` —— 选任务，更新 `pxCurrentTCB`；
8. `mov r0, #0` / `msr basepri, r0` —— 开中断；
9. `ldmia sp!, {r3, r14}` —— 恢复 TCB 指针与新 `EXC_RETURN`；
10. `ldr r1, [r3]` / `ldr r0, [r1]` —— 取**新任务**的 `pxTopOfStack`；
11. `ldmia r0!, {r4-r11}`（条件 `vldmia {s16-s31}`）；
12. `msr psp, r0` + `isb` —— PSP 指向新任务栈帧；
13. `bx r14` —— 以 `EXC_RETURN = 0xFFFFFFFD` 异常返回，硬件弹出 8 个字，新任务开始运行。
记忆口诀：**进异常硬件压 8 字 → 软件压 R4-R11 → 存栈顶 → 关中断选任务 → 换栈顶 → 软件弹 R4-R11 → 改 PSP → 异常返回硬件弹 8 字**。

### 42.如何切换 PSP？什么时候用 MSP？

PSP 只能靠 `MSR PSP, Rn`（读用 `MRS`）修改，FreeRTOS 只在两处改它：`vPortSVCHandler` 里 `msr psp, r0`（首次上车）与 `PendSV_Handler` 收尾的 `msr psp, r0`（每次切换）。切换 PSP 时**必须紧跟 `ISB`**，保证流水线里的后续指令看到新的栈指针（写 SP 类寄存器有同步要求）。
MSP 的使用时机：①复位到第一个任务启动之前（整个初始化都在 MSP 上跑）；②**所有异常/中断处理期间**（Handler 模式强制用 MSP，与任务用哪个栈无关）；③内核代码（`vTaskSwitchContext`、`xTaskIncrementTick` 等）都在 Handler 模式里跑，用的就是 MSP。所以“任务的栈”和“中断的栈”是两块独立内存，`configMINIMAL_STACK_SIZE` 管不了 ISR 的栈深度。

### 43.中断嵌套时会发生上下文切换吗？

**不会在嵌套过程中切换**，切换只可能在“中断返回到 Thread 模式之前”发生一次：
- ISR 里调 `...FromISR()` → 置 `PendSV` 挂起位，**不立即切**；
- 若发生嵌套，多个 ISR 会在 MSP 上叠自己的压栈帧，PendSV 因为优先级最低，**必须等所有 ISR 全部退出**才执行；
- 只有在外层 ISR 执行 `bx r14` 时，NVIC 发现 PendSV 挂起才**尾链（tail-chain）**到 PendSV，此时才是唯一一次真正的切换。
两条硬性约束（答出来显专业）：①所有调用 FreeRTOS API 的 ISR，**优先级数值必须 ≥ `configMAX_SYSCALL_INTERRUPT_PRIORITY`**，否则 BASEPRI 屏蔽不住它，会在内核操作链表时被打断而破坏数据（`portASSERT_IF_INTERRUPT_PRIORITY_INVALID()` 就是干这个）；②这类 ISR **必须用 `...FromISR()` 版本 API**。

### 44.PendSV 中再次触发 PendSV 会怎样？

**不会立刻重入**——同优先级的异常不能抢占正在执行的自身，ICC 只是把 `PENDSV` 挂起位置 1。行为是：
- PendSV 的 `bx r14` 异常返回时，NVIC 检查到 PendSV 又处于挂起态，于是**立刻再次尾链进入 PendSV**，相当于“多跑一轮切换”；
- 这对 FreeRTOS 是**安全且必要**的：`vTaskSwitchContext()` 里有一句 `if (uxSchedulerSuspended != 0) { xYieldPending = pdTRUE; }`——调度器被 `vTaskSuspendAll` 挂起时不能切换，就把“该切”的意愿记在 `xYieldPending` 里而不是乱切；等到 `xTaskResumeAll()` 再兑现。ISR 在临界区里多次置 PENDSVSET 也只是把同一个挂起位反复置 1，不会攒成多次切换（**挂起位是布尔，不是计数器**），所以“在一个 ISR 里连发 10 个 FromISR 唤醒”只会导致一次切换。
- 额外细节：PendSV 内部在调用 `vTaskSwitchContext` 前后用 BASEPRI 关中断，所以“PendSV 执行期间被 SysTick 打断”也不会破坏它。

### 45.SysTick 和 PendSV 同时挂起，谁先执行？

**PendSV 先执行**。两者优先级都被设成 `configKERNEL_INTERRUPT_PRIORITY`（同优先级），按 ARM 的规则，**同优先级时异常号小的先执行**：PendSV = 异常 14，SysTick = 异常 15 → PendSV 赢。
实践含义：SysTick 的 handler 里会调用 `xTaskIncrementTick()` 并**自己再置一次 PENDSVSET**，如果 PendSV 已经在挂起态，SysTick 执行完退出时再进 PendSV，结果是“一次 tick 处理 + 一次切换”，顺序天然正确；反过来若 SysTick 优先级高于 PendSV（有人改过），也不影响正确性，只是切换被推迟到 SysTick 之后——**这就是“PendSV 设最低”带来的顺序稳健性**。

### 46.临界区用 PRIMASK 还是 BASEPRI？为什么？

**优先用 BASEPRI**。原因：
- `PRIMASK`（`cpsid i`）屏蔽**所有**中断（除 NMI/HardFault），意味着即使是最紧急的、与 RTOS 无关的电机/通信中断也要等临界区结束，**直接把最坏中断延迟拉长到最长临界区的长度**；
- `BASEPRI` 只屏蔽“优先级数值 ≥ `configMAX_SYSCALL_INTERRUPT_PRIORITY`”的中断，也就是**只挡住会用 RTOS API 的那些中断，而把更高优先级（数值更小）的中断放行** —— 这就是 FreeRTOS 的“**零延迟中断（zero latency ISR）**”机制：这类中断可以随时响应，代价是**它们绝对不能调用任何 FreeRTOS API**。
- 例外：**ARMv6-M（Cortex-M0/M0+）没有 BASEPRI**，只能退回 PRIMASK（FreeRTOS 的 `portmacro.h` 里 `configMAX_SYSCALL_INTERRUPT_PRIORITY == 0` 时就走到 `cpsid i` 的分支），这也是 M0 端口性能差一截的原因。
- 实现细节：`taskENTER_CRITICAL()` 里嵌套计数用的是**每个任务 TCB 的 `uxCriticalNesting`**（`portCRITICAL_NESTING_IN_TCB == 1`），而 `..._FROM_ISR` 版本则靠“保存/恢复 BASEPRI 原值”实现嵌套，因为 ISR 没有 TCB 可用。

### 47.BASEPRI 设置成多少？会屏蔽哪些中断？

写入的是 **`configMAX_SYSCALL_INTERRUPT_PRIORITY = configLIBRARY_MAX_SYSCALL_INTERRUPT_PRIORITY << (8 - configPRIO_BITS)`**。举例（STM32，`configPRIO_BITS = 4`，典型 `configLIBRARY_MAX_SYSCALL_INTERRUPT_PRIORITY = 5`）：把 `5 << 4 = 0x50` 写进 BASEPRI。
屏蔽规则是“**比较高 8 位，屏蔽所有优先级数值 ≥ 0x50 的中断**”，也就是：
- **被屏蔽（数值 5~15）**：所有“与 RTOS 打交道”的中断（它们必须用 FromISR API）；
- **不被屏蔽（数值 0~4）**：高优先级/零延迟中断，**必须完全不调用 RTOS API**，否则会破坏内核链表（因为它们能在临界区内插进来）。
常见错误：把 `configLIBRARY_MAX_SYSCALL_INTERRUPT_PRIORITY` 设成 0（等于把 BASEPRI 写成 0 → 一个中断都屏不住，临界区形同虚设）；或者忘了 `(8 - configPRIO_BITS)` 的移位，写成了 `5` 而不是 `0x50`。

### 48.关中断时间怎么测？最长多少？

测量方法（两种都要说）：
- **GPIO + 示波器/逻辑分析仪**：在 `taskENTER_CRITICAL()` 前拉高、`taskEXIT_CRITICAL()` 后拉低（或在 BASEPRI 写入前后翻转），用示波器抓脉宽和密度；优点是直观、不扰动代码路径，缺点是要占用一个引脚且脉宽很短。
- **`DWT->CYCCNT`**：`DEMCR |= TRCENA` 后 `DWT->CYCCNT` 以 CPU 时钟计数，操作前后取值差即周期数，除以主频就是时间；开销仅几条指令，适合做“最坏值统计”（记录到全局 max 变量）。
- 最坏值不是常数：**内核最坏关中断时间与数据规模成正比**——`vListInsert()`（延时链表有序插入，O(n)）、`prvInsertBlockIntoFreeList()`（空闲链表插入/合并，O(空闲块数)）、`prvUnlockQueue()`（遍历阻塞链表）、`vTaskSwitchContext()` 通用版的优先级扫描。所以要在**压力场景**下测：延时任务最多、空闲块最碎、队列阻塞任务最多的时刻；否则测出来的是最好值。
- 对比参考：纯任务切换路径的关中断区间只有 PendSV 的第 6~8 步（`msr basepri` 到 `msr basepri, #0`），即 `vTaskSwitchContext` 一个函数的执行时间，通常几十个周期到一两百周期。

### 49.中断延迟怎么测？最坏多少？

定义要分清两个量：**①硬件中断延迟**（中断请求到 ISR 第一条指令）= NVIC 识别 + 压栈 12 周期（Cortex-M3 手册标称）+ 取向量与流水线，零等待 Flash 下约 12~16 个周期；**②系统级最坏中断延迟** = 硬件延迟 + **最长临界区关中断时间**，这才是面试官真正想问的，且它由第 48 题的“规模相关”项决定。
测法：
- 用定时器输出一个极窄脉冲（或另一个 GPIO 由软件翻转）作为“中断请求时刻”，在 ISR 的**第一条指令**里翻转一个 IO，示波器测量两个边沿的间隔 → 得到系统级延迟的实时分布（能直接抓到最长的那次）；
- 或者用 `DWT->CYCCNT`：GPIO 触发中断，ISR 里读 `CYCCNT` 与“触发前记录的值”相减；
- 想测“由临界区造成的额外延迟”，就把测量点放在临界区内部的某一处，配合第 48 题的方法。
- 如果用了**零延迟中断**（优先级高于 `configMAX_SYSCALL_INTERRUPT_PRIORITY`），它的延迟只有硬件那 12 个周期，**但代价是不能碰 RTOS API**——这个权衡是这道题的加分点。

### 50.HardFault 怎么定位？如何分析压栈帧？

标准手写 `HardFault_Handler` 的分析流程：
1. **先判断出错时用的是哪个栈**：读 `LR`（异常返回地址）的 bit2：`0xFFFFFFFD`（bit2=1）→ 用 PSP（**说明 Fault 发生在任务上下文**），从 `PSP` 取栈帧；`0xFFFFFFF9`（bit2=0）→ 用 MSP（**说明 Fault 发生在中断/内核上下文**）。汇编里要 `tst lr, #4` + `ite eq` 选择 `mrs r0, msp / mrs r0, psp`。
2. **从栈帧取现场**：栈帧布局（低→高）= `R0, R1, R2, R3, R12, LR, PC, xPSR`。**`PC`（第 6 个字）就是出错的那条指令的地址**（ARMv7-M 自带精确总线故障地址），用 `arm-none-eabi-addr2line -e xxx.elf <PC>` 或反汇编直接定位到函数与行号；`LR` 给出“谁调用的”。
3. **读故障状态寄存器定位原因**：`CFSR @0xE000ED28`（拆成 `MMFSR` 位 0-7、`BFSR` 位 8-15、`UFSR` 位 16-31）：`DIVBYZERO`、`UNALIGNED`、`INVSTATE`（**xPSR 的 Thumb 位丢了**——最常见于函数指针跳转或栈被冲掉）、`IMPRECISERR/PRECISERR`（取指/访问非法地址）、`BFARVALID` 时读 `BFAR @0xE000ED38` 拿到具体地址；`HFSR @0xE000ED2C` 的 `FORCED` 位说明“是被升级上来的 Fault”。**注意 `CFSR` 是写 1 清零的，读取要在处理里做记录**。
4. **结合 RTOS 定位任务**：打印 `pxCurrentTCB->pcTaskName`、`uxTaskGetStackHighWaterMark()`、`vTaskList()` 的现场快照，能迅速判断是不是栈溢出（配合 `configCHECK_FOR_STACK_OVERFLOW`）。
5. 工程实践：把这一套做成“**把现场存到 noinit RAM + 复位 + 下次启动上报**”，比当场死循环更利于现场问题定位。

### 51.如果移植到 Cortex-M0 或 M4，需要改哪些地方？

**M0（ARMv6-M）**：换到 `portable/GCC/ARM_CM0`，需要改的是——`portmacro.h` 的临界区宏退化为 PRIMASK（无 BASEPRI）、`configUSE_PORT_OPTIMISED_TASK_SELECTION` 必须关（无 CLZ）、汇编不能再用 IT 块/多寄存器条件执行（`portasm` 另写一份）、`__dsb/__isb` 用 CMSIS 同名函数、无硬件除法/未对齐访问要注意，另外部分 M0 只有 2 位优先级 → `configPRIO_BITS` 要跟着改。**结论：内核 C 代码一行不改，改的全在 port 层与 FreeRTOSConfig.h**。
**M4/M4F**：换到 `ARM_CM4F`，改动——①汇编里增加 FPU 上下文（按 `EXC_RETURN` 的 FS 位条件 `vstmdb/vldmia {s16-s31}`）；②启动代码里开 `CPACR`（CP10/CP11 置全权）；③配置 `configUSE_TASK_FPU_SUPPORT` 决定“每个任务是否独立拥有浮点上下文”（=0 关闭、=1 惰性按需、=2 强制保存，注意 **=2 会让所有任务都背 132 字节浮点上下文**）；④数学库要用带 FPU 的 ABI（`-mfloat-abi=hard -mfpu=fpv4-sp-d16`），否则白搭；⑤若同时用 MPU，还要处理 MPU 端口的分支。

## 四、任务间同步与通信

### 52.消息队列控制块包含哪些字段？

FreeRTOS `Queue_t`（32 位约 80 字节，队列存储区**紧随其后同一块内存**）：
- `pcHead` / `pcTail`：存储区起止边界（用于指针回绕判断）；
- `pcWriteTo`：下一个写入位置；`u.xQueue.pcReadFrom`：下一个读取位置（用 union 与信号量的 `xMutexHolder` 复用同一块空间，**省 RAM 的经典手法**）；
- `xTasksWaitingToSend` / `xTasksWaitingToReceive`：**两个事件链表**，分别挂“队列满而阻塞的发送者”和“队列空而阻塞的接收者”，按优先级排序；
- `uxMessagesWaiting`：**当前队列中消息个数**（空/满判断的唯一依据）；
- `uxLength`：队列容量（项数）；`uxItemSize`：**每项字节数（0 表示是信号量/互斥量）**；
- `cRxLock` / `cTxLock`：**队列锁计数器**（见第 74 题），ISR 在队列被锁时不操作链表，只通过它记账，把链表操作推迟到任务上下文；
- `ucQueueType`：队列类型（`queueQUEUE_TYPE_BASE/BINARY_SEMAPHORE/COUNTING_SEMAPHORE/MUTEX/RECURSIVE_MUTEX`），调试用；
- `uxQueueNumber`/`uxQueueNumber`、`pcQueueName`、`uxQueueNumber` 等调试字段（`configQUEUE_REGISTRY_SIZE` 时）。

### 53.环形缓冲区读写指针怎么管理？空和满怎么判断？

FreeRTOS 队列的做法非常“省心”：**维护“项数”而不是“头尾比较”**。
- **空**：`uxMessagesWaiting == 0`；**满**：`uxMessagesWaiting == uxLength`。因为写之前先判满、读之后 `uxMessagesWaiting--`，不存在“头尾指针相等时分不清空满”的经典歧义（那个问题只出现在“只靠头尾指针判断”的实现里，解法是牺牲一格或加计数器）。
- 指针推进：`pcWriteTo += uxItemSize; if (pcWriteTo >= pcTail) pcWriteTo = pcHead;`（写满一圈后回绕）；读同理用 `pcReadFrom`，**队列中项的顺序天然由“项数 + 指针”保证，不需要移动已有数据**。
- 变体：`xQueueSendToFront` 的写入点是 `pcReadFrom` 往回退一项（实现插队/优先级消息）；`xQueueOverwrite` 在满时先 `--uxMessagesWaiting` 再覆盖最旧项。
- 对比流缓冲区（`stream_buffer.c`）：那里才是真正的**字节流**环形缓冲，用 `xHead/xTail` 指针 + 环绕处理 + `xStreamBufferBytesAvailable()` 计算可用量，并需要显式处理回绕的分段拷贝。

### 54.队列项大小固定吗？是拷贝数据还是传指针？

**创建时固定**（`uxItemSize` 决定每项字节数与内存步进），因此**同一队列不能混投不同大小的消息**，运行时不能改。数据是**按值拷贝**（`prvCopyDataToQueue` 里的 `memcpy(pcWriteTo, pvItemToQueue, uxItemSize)`），接收侧同样拷贝出来 —— 代价是“消息越大，拷贝越贵且必须在临界区外尽快完成”。
所以工程惯例是：**大消息发指针（队列项 = 指针），小块数据（几个字节的状态/指令）直接拷值**。发指针时必须自己解决生命周期：接收方用完谁释放（常用“带内存池的指针队列”或“回收队列”），并且**绝不能让栈上局部变量的地址进队列**（那个栈随时会被覆盖）。要传变长数据就换 `xMessageBuffer`（FreeRTOS 的 `stream_buffer.c` 支持变长消息 + 带长度前缀）。

### 55.队列满时发送任务阻塞怎么实现？

`xQueueGenericSend()` 的关键路径（`xTicksToWait != 0` 时）：
- 任务已经在临界区内确认队列满；
- **若是互斥量且当前持有者优先级更低 → 先 `xTaskPriorityInherit(pxQueue->u.xSemaphore.xMutexHolder)`**（这条只对互斥量有效，见第 63 题）；
- `vTaskPlaceOnEventList(&pxQueue->xTasksWaitingToSend, xTicksToWait)`：把当前 TCB 的 `xEventListItem` 按“优先级取反值”**有序插入**阻塞链表，同时 `prvAddCurrentTaskToDelayedList()` 把它按“超时时刻”挂进延时链表（`xTicksToWait == portMAX_DELAY` 则不挂延时表）；
- `prvUnlockQueue(pxQueue)`（把 `cTxLock` 期间 ISR 攒下的唤醒动作补做）；
- `portYIELD_WITHIN_API()` 触发 PendSV 让出 CPU。
唤醒路径：接收方取走一项后，若 `xTasksWaitingToSend` 非空则 `xTaskRemoveFromEventList()` 把**阻塞链表头部（= 最高优先级）的发送者**移出并挂回就绪表，必要时 `portYIELD_WITHIN_API()`。
